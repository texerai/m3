...
{
    // SignalMemopCommitHookBoomBridge(hart_id, is_load, rob_id, clock_cycle);
}
...
{
    SignalStoreCompletedBoomBridge(hart_id, store_address, store_data, stq_idx, clock_cycle);
}
...
{
    SignalAddStoreAddressBoomBridge(hart_id, store_address, store_size, rob_id, stq_idx, clock_cycle);
}
...
{
    SignalAddLoadAddressBoomBridge(hart_id, load_address, load_size, rob_id, ldq_idx, clock_cycle);
}
...
{
    SignalMemopInsertHookBoomBridge(hart_id, rob_id, raw_instruction, is_load, clock_cycle);
}
...
void SignalAddMemopAddressHookAPI
(
    dromajo_cosim_state_t* state,
    int hart_id,
    int rob_id,
...
{
    SignalAddMemopAddressHookBoomBridge(hart_id, rob_id, is_load, address, memop_size, clock_cycle);
}
...
{
    SignalPerformLoadHookBoomBridge(hart_id, rob_id, loaded_data, clock_cycle);
}
...
{
    SignalStoreLocalPeformBoomBridge(hart_id, rob_id, is_amo, stq_idx, local_store_address, local_store_data, clock_cycle);
}
...
{
    SignalStoreNukeBoomBridge(hart_id, rob_id, stq_idx, clock_cycle);
}
...
{
    SignalStoreCommitBoomBridge(hart_id, rob_id, stq_idx, clock_cycle);
}
...
{
    SignalStoreGlobalPerformFromMetaWriteArbiterBoomBridge(hart_id, address, tag, idx, coherence_state, meta_way_en, data_way_en, data, meta_write_valid, data_write_valid, clock_cycle);
}
...
{
    dromajo_cosim_raise_trap(state, hart_id, ((is_interrupt << 63) | cause_code));
}
                       ...
                       bool                   check)
{
    ...
    // Marionette memory model handlings.
    uint64_t performed_data = 0;
    bool is_memop_commit_sucess = true;
    bool is_insn_amo = is_amo(emu_insn);
    if (RVUtils::is_memop(emu_insn) || is_insn_amo) {
        // Get the destination register.
        int rd     = (emu_insn >> 7) & 0x1f;
        int rdc    = ((emu_insn >> 2) & 0x7) + 8;

        int32_t prev_rob_id = rob_id - 1;
        if (prev_rob_id < 0) {
            prev_rob_id = 31;
        }

        bool is_ld = RVUtils::is_load(emu_insn) || RVUtils::is_compressed_load(emu_insn);
        is_memop_commit_sucess = SignalMemopCommitHookBoomBridge(hartid, is_ld, is_insn_amo, prev_rob_id, clock_cycle, &performed_data);

        if (is_memop_commit_sucess) {
            if (RVUtils::is_compressed_load(emu_insn)) {
                riscv_set_reg(s, rdc, performed_data);
                iregno = rdc;
            } else if (RVUtils::is_load(emu_insn)) {
                riscv_set_reg(s, rd, performed_data);
                iregno = rd;
            }
        }
    }
    ...
    if (emu_pc      != dut_pc                           ||
        ...
        emu_wdata   != dut_wdata && emu_wrote_data || !is_memop_commit_sucess) {
...
// Wrapper step function for auto generated hook.
static uint32_t insn_after_failure = 3;
static bool kill_soon = false;
...
{
    int code = dromajo_cosim_step(state, hart_id, rob_id, clock_cycle, pc, raw_instruction, write_data, 0, true);
    if (code != 0)
    {
        kill_soon = true;
    }
