...
/*
 * dromajo_cosim_update_register --
 *
 * updates the architectural register.
 */
void dromajo_cosim_update_register(dromajo_cosim_state_t *state, int hartid, uint32_t rd, uint64_t data, bool is_fp)
{
    ...
    if (is_fp)
    {
        riscv_set_fpreg(s, rd, data);
    }
    else
    {
        riscv_set_reg(s, rd, data);
    }
}
...
int dromajo_cosim_step(dromajo_cosim_state_t *state, int hartid, uint64_t dut_pc, uint32_t dut_insn, uint64_t dut_wdata,
    ...
    log_buff_space = 0;
    if (verbose) {
        log_buff_space += snprintf(log_buffer+log_buff_space, 512-log_buff_space, "C%d %d 0x%016" PRIx64 " ", hartid, emu_priv, emu_pc);
        log_buff_space += snprintf(log_buffer+log_buff_space, 512-log_buff_space, "(0x%08x) ", emu_insn);
    }
