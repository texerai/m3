...
namespace m3
{
    ...
    struct Cache
    {
        ...
        void UpdateMetaData(uint32_t way_id, uint32_t cache_line_id, uint32_t new_state, uint32_t new_tag)
        {
            ...
            //HASH coming from RTL, identifying llc slice, llc way, llc set.
        }
    };
    ...
    struct State
    {
        ...
        // Map STQ to rbid entries, the ones that will be written to mem, they are merged in st global. STQ IDs are stored beyond commit.
        std::unordered_map<uint32_t, std::unordered_map<uint32_t, std::vector<Inst_id>>> beyond_core_stores;
