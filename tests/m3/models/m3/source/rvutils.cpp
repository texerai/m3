...
bool RVUtils::is_store(uint32_t insn)
{
    ...
    bool is_c_store = is_compressed_store(insn);
    return (is_i_store || is_f_store || is_c_store);
}
...
bool RVUtils::is_load(uint32_t insn)
{
    ...
    bool is_c_load = is_compressed_load(insn) || is_compressed_fp_load(insn);
    return (is_i_load || is_f_load || is_c_load);
}
...
bool RVUtils::is_amo_add(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_swap(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_xor(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_and(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_or(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_min(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_max(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_minu(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
bool RVUtils::is_amo_maxu(uint32_t insn)
{
    int opcode = insn & 0x7f;
    if (opcode != 0x2f)
        return false;
...
// 32-bit AMO operations
uint32_t RVUtils::add_w(uint32_t mem_value, uint32_t st_value) {
    return mem_value + st_value;
}

uint32_t RVUtils::swap_w(uint32_t mem_value, uint32_t st_value) {
    return st_value;
}

uint32_t RVUtils::xor_w(uint32_t mem_value, uint32_t st_value) {
    return mem_value ^ st_value;
}

uint32_t RVUtils::and_w(uint32_t mem_value, uint32_t st_value) {
    return mem_value & st_value;
}

uint32_t RVUtils::or_w(uint32_t mem_value, uint32_t st_value) {
    return mem_value | st_value;
}

uint32_t RVUtils::min_w(uint32_t mem_value, uint32_t st_value) {
    return std::min(static_cast<int32_t>(mem_value), static_cast<int32_t>(st_value));
}

uint32_t RVUtils::max_w(uint32_t mem_value, uint32_t st_value) {
    return std::max(static_cast<int32_t>(mem_value), static_cast<int32_t>(st_value));
}

uint32_t RVUtils::minu_w(uint32_t mem_value, uint32_t st_value) {
    return std::min(mem_value, st_value);
}

uint32_t RVUtils::maxu_w(uint32_t mem_value, uint32_t st_value) {
    return std::max(mem_value, st_value);
}

// 64-bit AMO operations
uint64_t RVUtils::add_d(uint64_t mem_value, uint64_t st_value) {
    return mem_value + st_value;
}

uint64_t RVUtils::swap_d(uint64_t mem_value, uint64_t st_value) {
    return st_value;
}

uint64_t RVUtils::xor_d(uint64_t mem_value, uint64_t st_value) {
    return mem_value ^ st_value;
}

uint64_t RVUtils::and_d(uint64_t mem_value, uint64_t st_value) {
    return mem_value & st_value;
}

uint64_t RVUtils::or_d(uint64_t mem_value, uint64_t st_value) {
    return mem_value | st_value;
}

uint64_t RVUtils::min_d(uint64_t mem_value, uint64_t st_value) {
    return std::min(static_cast<int64_t>(mem_value), static_cast<int64_t>(st_value));
}

uint64_t RVUtils::max_d(uint64_t mem_value, uint64_t st_value) {
    return std::max(static_cast<int64_t>(mem_value), static_cast<int64_t>(st_value));
}

uint64_t RVUtils::minu_d(uint64_t mem_value, uint64_t st_value) {
    return std::min(mem_value, st_value);
}

uint64_t RVUtils::maxu_d(uint64_t mem_value, uint64_t st_value) {
    return std::max(mem_value, st_value);
}
