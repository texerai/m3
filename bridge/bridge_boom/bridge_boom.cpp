...
namespace m3
{
        ...
        static bool CommitMemop(const RtlHookData& data, State& state)
        {
            ...
            if (memop_info.memop_type == MemopType::kLoad)
            {
                if (!memop_info.check_failed)
                {
                    ...
                    // (data.hart_id, rd, memop_info.load_rtl_data, is_fp);
                }
                else
                {
                    should_abort = true;
    ...
    struct BridgeBoom::BridgeBoomImpl
    {
        ...
        std::function<void(uint32_t, uint32_t, uint64_t, bool)> UpdateProcessorRegister;
    };
    ...
    {
        pimpl_->UpdateProcessorRegister = update_proc_register;
    }
}